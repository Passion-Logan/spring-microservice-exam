 ...
 * LoadingCache工具类，单例
 * <p>
 * 用法： 1. LoadingCacheHelper.getInstance().getOrDefault(SimpleCacheLoader.class, "key", "defaultValue"); 2.
 * LoadingCacheHelper.getInstance().get(SimpleCacheLoader.class, "key");
 * <p>
 *
 * @author WQL
 * @since 2020年1月16日 0016 16:30
 */
@Slf4j
public class LoadingCacheHelper {

    public static final ListeningExecutorService REFRESH_POOLS = MoreExecutors.listeningDecorator(
        Executors.newFixedThreadPool(1, new ThreadFactoryBuilder().setNameFormat("refresh-cache-thread-%d").build()));
    ...
    public static LoadingCacheHelper getInstance() {
        return LoadingCacheHelperInstance.instance;
    }
